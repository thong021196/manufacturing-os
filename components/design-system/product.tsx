...
export function EngineeringSpecGrid({ items }: { items: Array<{ label: string; value: string; annotation?: string }> }) {
  return <div className="spec-grid">{items.map((item) => <div key={item.label} className="spec-grid__item"><span>{item.label}</span><strong>{item.value}</strong>{item.annotation && <small>{item.annotation}</small>}</div>)}</div>;
}

export function TechnicalSpecTable({ rows }: { rows: Array<{ label: string; value: string; status?: string }> }) {
  return <div className="spec-table-wrap"><table className="spec-table"><tbody>{rows.map((row) => <tr key={row.label}><th>{row.label}</th><td>{row.value}</td><td>{row.status && <StatusTag tone={row.status === "verified" ? "green" : "blue"}>{row.status}</StatusTag>}</td></tr>)}</tbody></table></div>;
}

export function CriticalFeatureList({ features }: { features: Array<{ index: string; title: string; detail: string }> }) {
  return <div className="annotation-list">{features.map((feature) => <Annotation key={feature.index} index={feature.index} title={feature.title}>{feature.detail}</Annotation>)}</div>;
}

export function ProcessRoute({ steps }: { steps: Array<{ step: string; title: string; detail: string }> }) {
  return <div className="process-route">{steps.map((step, index) => <div key={step.step} className="process-step"><span>{step.step}</span><div><strong>{step.title}</strong><p>{step.detail}</p></div>{index < steps.length - 1 && <div className="process-step__line" aria-hidden="true" />}</div>)}</div>;
}

export function InspectionMatrix({ rows }: { rows: Array<{ check: string; method: string; evidence: string }> }) {
  return <TechnicalPanel label="QUALITY / INSPECTION"><div className="inspection-intro"><Shield size={22} /><div><h3>Inspection plan is scoped to the drawing</h3><p>Final acceptance criteria are confirmed against the released revision and customer requirements.</p></div></div><TechnicalSpecTable rows={rows.map((row) => ({ label: row.check, value: `${row.method} · ${row.evidence}`, status: "verified" }))} /></TechnicalPanel>;
}
...
export function RelatedEntityGrid({ title, items }: { title: string; items: Array<{ label: string; type: string; href?: string; detail: string }> }) {
  return <div><SectionHeading title={title} /><div className="entity-grid">{items.map((item) => <Link key={item.label} href={item.href ?? "#"} className="entity-card"><span>{item.type}</span><strong>{item.label}</strong><p>{item.detail}</p><ArrowUpRight size={15} /></Link>)}</div></div>;
}
