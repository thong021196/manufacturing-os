...
const initialDraft: RfqDraft = {
  ...
  material: "", finish: "", requirements: "", email: "", company: "", contactNote: "",
};
          ...
          <div className="mx-console__card">
              ...
              <div className="mx-console__form">
                ...
                {draft.files.length > 0 && (
                  <div className="mx-file-chips" aria-live="polite">
                    {draft.files.map((file) => <span key={`${file.name}-${file.lastModified}`}><Check size={13} /> {file.name}</span>)}
                  </div>
                )}
            ...
            {step === 1 && (
              <div className="mx-console__form">
                ...
                <div className="mx-console__form-grid">
                  <label>Quantity
                    <input value={draft.quantity} onChange={(event) => updateDraft("quantity", event.target.value)} type="number" placeholder="Prototype / production quantity" />
                  </label>
                  <label>Target date
            ...
            {step === 3 && (
              <div className="mx-console__form">
                <div className="mx-console__form-grid">
                  <label>Work email
                    <input value={draft.email} onChange={(event) => updateDraft("email", event.target.value)} type="email" placeholder="you@company.com" />
                  </label>
                  <label>Company
            ...
            <div className="mx-console__actions">
              <Button type="button" variant="ghost" onClick={() => setStep((current) => Math.max(0, current - 1))} disabled={step === 0}>Back</Button>
              <Button type="button" onClick={() => (step === steps.length - 1 ? setSubmitted(true) : setStep((current) => Math.min(steps.length - 1, current + 1)))}>
